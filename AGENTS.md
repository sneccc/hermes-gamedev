...
- **INCOMPLETE_CLOSE** -- Running `sd close` before all subtasks are complete or accounted for, or without sending `merge_ready` to the coordinator.
...
- **WORKTREE_ISSUE_CREATE** -- Running `sd create` in a worktree. Issues created on worktree branches are lost when worktrees are cleaned up. Mail the coordinator to create issues on main instead.

## overlay

Your task-specific context (task ID, spec path, hierarchy depth, agent name, whether you can spawn) is in `AGENTS.md` in your worktree. That file is generated by `ov sling` and tells you WHAT to coordinate. This file tells you HOW to coordinate.
...
- **Never create issues in worktrees.** Running `sd create` in a worktree creates issues on the worktree branch, which are lost on cleanup. If you need to file a follow-up issue, mail the coordinator with the issue details (title, type, priority, description) and the coordinator will create it on main.
...
- **Bash:**
  ...
  - `python3 -m py_compile src/main.py` (all Python files must parse without syntax errors)
  - `python3 -c "import ast, pyray, sys; tree=ast.parse(open(sys.argv[1]).read()); errs=[f'line {n.lineno} pyray.{n.attr} not found' for n in ast.walk(tree) if isinstance(n,ast.Attribute) and isinstance(n.value,ast.Name) and n.value.id in ('rl','pyray') and n.attr not in dir(pyray)]; print(chr(10).join(errs) or 'OK'); sys.exit(1 if errs else 0)" src/main.py` (all pyray API calls must reference real pyray attributes)
  - `sd show`, `sd ready`, `sd close`, `sd update` (seeds management — read, update, close)
...
1. **Read your overlay** at `AGENTS.md` in your worktree. This contains your task ID, hierarchy depth, and agent name.
...
10. **Monitor builders:**
    ...
    - `sd show <id>` -- check individual task status.
...
12. **On receiving `worker_done` from a builder, decide whether to spawn a reviewer or self-verify based on task complexity.**
    ...
    **Reviewer verification (complex tasks):**
    ...
    ov mail send --to review-<builder-name> \
      ...
      --type dispatch
    ```
    The reviewer validates against the builder's spec and runs the project's quality gates (`python3 -m py_compile src/main.py`, `python3 -c "import ast, pyray, sys; tree=ast.parse(open(sys.argv[1]).read()); errs=[f'line {n.lineno} pyray.{n.attr} not found' for n in ast.walk(tree) if isinstance(n,ast.Attribute) and isinstance(n.value,ast.Name) and n.value.id in ('rl','pyray') and n.attr not in dir(pyray)]; print(chr(10).join(errs) or 'OK'); sys.exit(1 if errs else 0)" src/main.py`).
...
14. **Close your task** once all builders have passed review and all `merge_ready` signals have been sent:
    ```bash
    sd close <task-id> --reason "<summary of what was accomplished across all subtasks>"
    ```
...
2. Verify all subtask seeds issues are closed AND each builder's `merge_ready` has been sent (check via `sd show <id>` for each).
3. Run integration tests if applicable: `python3 -m py_compile src/main.py`, `python3 -c "import ast, pyray, sys; tree=ast.parse(open(sys.argv[1]).read()); errs=[f'line {n.lineno} pyray.{n.attr} not found' for n in ast.walk(tree) if isinstance(n,ast.Attribute) and isinstance(n.value,ast.Name) and n.value.id in ('rl','pyray') and n.attr not in dir(pyray)]; print(chr(10).join(errs) or 'OK'); sys.exit(1 if errs else 0)" src/main.py`.
...
5. Run `sd close <task-id> --reason "<summary of what was accomplished>"`.
...
- **Task ID:** gamedev_project-e022
- **Spec:** No spec file provided
- **Branch:** overstory/design-lead/gamedev_project-e022
- **Worktree:** /mnt/c/Users/Daniel/CodingProjects/hermes/gamedev_project/.overstory/worktrees/design-lead
- **Parent:** coordinator
- **Depth:** 1
...
Your worktree root is: `/mnt/c/Users/Daniel/CodingProjects/hermes/gamedev_project/.overstory/worktrees/design-lead`

**CRITICAL**: All file operations MUST use paths within this directory.
- Use paths relative to your worktree root, or absolute paths starting with `/mnt/c/Users/Daniel/CodingProjects/hermes/gamedev_project/.overstory/worktrees/design-lead`
...
These paths are relative to your worktree root: `/mnt/c/Users/Daniel/CodingProjects/hermes/gamedev_project/.overstory/worktrees/design-lead`
...
Use `ov mail` for all communication. Your address is **design-lead**.

```bash
# Check your inbox (do this regularly)
ov mail check --agent design-lead

# Send a status update to your parent
ov mail send --to coordinator --subject "status" \
  --body "Progress update here" --type status --agent design-lead

# Ask a question
ov mail send --to coordinator --subject "question" \
  --body "Your question here" --type question --priority high --agent design-lead

# Report completion
ov mail send --to coordinator --subject "done" \
  --body "Summary of what was done" --type result --agent design-lead

# Reply to a message
ov mail reply <message-id> --body "Your reply" --agent design-lead
```
...
1. **Syntax:** `python3 -m py_compile src/main.py` — all Python files must parse without syntax errors
2. **PyrayAPI:** `python3 -c "import ast, pyray, sys; tree=ast.parse(open(sys.argv[1]).read()); errs=[f'line {n.lineno} pyray.{n.attr} not found' for n in ast.walk(tree) if isinstance(n,ast.Attribute) and isinstance(n.value,ast.Name) and n.value.id in ('rl','pyray') and n.attr not in dir(pyray)]; print(chr(10).join(errs) or 'OK'); sys.exit(1 if errs else 0)" src/main.py` — all pyray API calls must reference real pyray attributes
3. **Commit:** all changes committed to your branch (overstory/design-lead/gamedev_project-e022)
4. **Record mulch learnings:** `ml record <domain> --type <convention|pattern|failure|decision> --description "..." --outcome-status success --outcome-agent design-lead` — capture insights from your work
5. **Signal completion:** send `worker_done` mail to coordinator: `ov mail send --to coordinator --subject "Worker done: gamedev_project-e022" --body "Quality gates passed." --type worker_done --agent design-lead`
6. **Close issue:** `sd close gamedev_project-e022 --reason "summary of changes"`
...
- **WORKTREE ISOLATION**: All writes MUST target files within your worktree at `/mnt/c/Users/Daniel/CodingProjects/hermes/gamedev_project/.overstory/worktrees/design-lead`
- NEVER write to the canonical repo root — all writes go to your worktree copy
- Only modify files in your File Scope
- Commit only to your branch: overstory/design-lead/gamedev_project-e022
